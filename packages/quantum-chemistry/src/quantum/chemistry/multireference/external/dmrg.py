...
class DMRGMethod(MultireferenceMethod):
    ...
    def _validate_block2(self):
        ...
        try:
            ...
            from block2 import SpinLabel, SZ, VectorUInt8
            from block2.su2 import HamiltonianQC, SimplifiedMPO, DMRG
    ...
    def _run_dmrg_casscf(self,
        ...
        try:
            ...
            from block2 import init_memory_pool, release_memory_pool
            from block2 import Threading, set_omp_threads
            from block2.su2 import HamiltonianQC, SimplifiedMPO, DMRG
            ...
            try:
                ...
                h1e, h2e, ecore = self._get_active_space_integrals(scf_obj, active_space)
                ...
                converged = len(dmrg.energies) < self.max_sweeps or \
                          abs(dmrg.energies[-1] - dmrg.energies[-2]) < self.davidson_threshold
                ...
                result = {
                    ...
                    'discarded_weight': dmrg.discarded_weights[-1] if dmrg.discarded_weights else 0.0,
    ...
    def _get_active_space_integrals(self,
                                   scf_obj: Union[scf.hf.SCF, scf.uhf.UHF],
                                   active_space: ActiveSpaceResult) -> Tuple[np.ndarray, np.ndarray, float]:
        """
    ...
    def _apply_post_correction(self,
        ...
        if self.post_correction.lower() == 'nevpt2':
            return self._apply_nevpt2_correction(dmrg_result, scf_obj, active_space, **kwargs)
        elif self.post_correction.lower() == 'caspt2':
            return self._apply_caspt2_correction(dmrg_result, scf_obj, active_space, **kwargs)
        else:
            raise ValueError(f"Unknown post-correction method: {self.post_correction}")
    
    def _apply_nevpt2_correction(self,
                                dmrg_result: Dict[str, Any],
                                scf_obj: Union[scf.hf.SCF, scf.uhf.UHF],
                                active_space: ActiveSpaceResult,
                                **kwargs) -> Dict[str, Any]:
    ...
    def _apply_caspt2_correction(self,
                                dmrg_result: Dict[str, Any],
                                scf_obj: Union[scf.hf.SCF, scf.uhf.UHF],
                                active_space: ActiveSpaceResult,
                                **kwargs) -> Dict[str, Any]:
